...
public class AuthService {
    ...
    public String register(UserDTO user) throws LoginException {
        ...
        String hashedPassword = hashService.hashPassword(user.getPassword());

        userE.setPasswordHash(hashedPassword);

        if (!authDao.addUser(userE)){
            throw new LoginException("Данный логин уже существует!!");
        }
